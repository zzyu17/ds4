...
static bool parse_reasoning_effort_name(const char *s, ds4_think_mode *out) {
    ...
    if (!strcmp(s, "xhigh") || !strcmp(s, "high")) {
        /* Non-official xhigh is kept as the existing high alias. */
        *out = DS4_THINK_HIGH;
        return true;
    }
    if (!strcmp(s, "medium")) {
        /* Qwen uses a distinct named medium mode; GLM maps it to high. */
        *out = DS4_THINK_MEDIUM;
        return true;
    }
    if (!strcmp(s, "low") || !strcmp(s, "minimal")) {
        /* DeepSeek V4 low is the default, prefix-free effort; "minimal" aliases it. */
        *out = DS4_THINK_LOW;
        return true;
    }
    if (!strcmp(s, "none")) {
...
static void test_reasoning_effort_mapping(void) {
    ...
    TEST_ASSERT(ds4_glm_reasoning_effort_text(DS4_THINK_LOW) == NULL);
